<?php
include_once './../inc/constants.php';
include_once './../inc/connection.php';
include_once './../inc/functions.php';
include_once './../inc/header.php';
include_once './../inc/sidebar.php';

//Check if admin is logged out or not
admin_logged_out();

// Update Record on submit
if(isset($_POST['data_update'])) {

    $product_old_img = $_POST['product_old_img'];
    $field_name = $_POST['field_name'];
    $field_description = $_POST['field_description'];
    $field_category = $_POST['field_category'];
    $product_id = $_POST['product_id'];
    $updated_date = date("Y-d-m h:i:s", time());

    if($_FILES['update_field_file']['error'] != UPLOAD_ERR_NO_FILE ) {
        // Get file name and rename file before uploading.
        $file_get = ($_FILES['update_field_file']['name']);
        $file_temp_name = ($_FILES['update_field_file']['tmp_name']);
        $file_name_arr = explode(".",$file_get);
        $file_name = $file_name_arr[0];
        $file_name = preg_replace('/\s+/', '', $file_name);
        $file_name = trim($file_name);
        $extension = end($file_name_arr);

        // Check file size
        if($_FILES['update_field_file']['size']>= 1048576) {
            $file_err = "<span class='text-danger'>File size must be less than 1mb.</span>";
        }
        else {
            unset($file_err);
            $file_new_name = $file_name.'_'.strtotime(date('Y-m-d H:i:s')).'.'.$extension;
            $file_to_saved = "./../assets/images/product_images/".$file_new_name;
            unlink("./../assets/images/product_images/".$product_old_img);

        }
    }
    else {
        $file_new_name = $product_old_img;
    }

    // Once the file is validated update the record and move resized file to server.
    if(empty($file_err)) {
        $upload_file =  move_uploaded_file($file_temp_name,$file_to_saved);
        $update_qry = "update product set category_p_id='$field_category' , product_name='$field_name' , product_description='$field_description' , product_image='$file_new_name' , updated_date = '$updated_date' where product_id='$product_id'";
        $update_qry_result = mysqli_query($con,$update_qry);

        // Success message
        if($update_qry_result && $upload_file) {
            header("location:?product_id=$product_id&success");
        }
    }

}


// Function to print edit form
function print_edit_form($product_id) {
    global $con;
    global $file_err;
    $fetch_record_qry = "select * from product where product_id='$product_id'";
    $fetch_record_qry_result = mysqli_query($con,$fetch_record_qry);
    $record_arr = mysqli_fetch_array($fetch_record_qry_result);

    if(isset($_REQUEST['success'])) {
        print_success_message("update");
    }
    ?>
    <form method="post" action="<?php echo $_SERVER['PHP_SELF']."?product_id=".$record_arr['product_id'].""; ?>" id="admin-site-form" enctype="multipart/form-data">
        <div class="form-group">
            <label for="field_name">Product Name</label>
            <input type="text" class="form-control" id="field_name" placeholder="Product Name" name="field_name" value="<?php echo $record_arr['product_name']; ?>">
        </div>
        <div class="form-group">
            <label>Select Category</label>
            <select class="form-control" id="field_category" name="field_category">
                <?php
                    $product_name_qry = "select * from category where category_id='$record_arr[category_p_id]'";
                    $product_name_qry_result = mysqli_query($con,$product_name_qry);
                    $product_name_arr = mysqli_fetch_array($product_name_qry_result);
                ?>
                <option value="<?php echo $product_name_arr['category_id']; ?>" selected><?php echo $product_name_arr['category_name']; ?></option>
                <?php
                print_category_dropdown();
                ?>
            </select>
        </div>
        <div class="form-group">
            <label for="field_description">Product Description</label>
            <input type="text" class="form-control" id="field_description" placeholder="Product Description" name="field_description" value="<?php echo $record_arr['product_description']; ?>">
        </div>

        <div class="form-group">
            <label class="form-label" for="customFile">Default file input example</label>
            <input type="file" class="form-control" id="customFile field_file" name="update_field_file">
            <?php if(isset($file_err)) echo $file_err; ?>
        </div>
        <input type="hidden" value="<?php echo $record_arr['product_image']; ?>" name="product_old_img">
        <input type="hidden" value="<?php echo $product_id; ?>" name="product_id">
        <button type="submit" class="btn btn-primary" name="data_update">Submit</button>
    </form>
    <?php
}
?>
<div class="container table-container">
    <h1 class="text-center font-weight-light pt-4">Edit Product</h1>
    <?php
    if(isset($_REQUEST['product_id'])) {
        $product_id = $_REQUEST['product_id'];
        print_edit_form($product_id);
    }
    else {
        header("location:./index/");
    }
    ?>
</div>


<?php
include_once './../inc/sidebar-end.php';
include_once './../inc/footer.php';
?>
